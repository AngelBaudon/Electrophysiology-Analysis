...
class PlotFactory():
    ...
    def Min_Max_Ploter(self, X_label, Y_label, drug, drug_time, sampling_Hz,
                       event_type, max_min_traces, Max_Indx, Min_Indx,
                       behavior, show_fig = False):
        
        
        #Plot the trace with the Maximum & Minimum frames
        plt.figure(figsize = (15,10))
        plot, loc = plt.subplot(311), 4
        ...
        for Start, Stop in Min_Indx: plt.axvspan(Start, Stop, alpha=0.25, color='lightcoral')
        
        for f, frame in enumerate((Max_Indx, Min_Indx)):
            for (fr1, fr2) in frame:
                Start, Stop = int(fr1*sampling_Hz), int(fr2*sampling_Hz)
                interv, loc = plt.subplot(3,3,loc), loc+1
        ...
        pickle.dump(plot, open(rf'{max_min_traces}/PLT/{behavior}_{self.file_name}', 'wb'))
        plt.savefig(rf'{max_min_traces}/PDF/{behavior}_{self.file_name}.pdf')
        if not show_fig: plt.close()
